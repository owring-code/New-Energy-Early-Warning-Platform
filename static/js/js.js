...
function combinedChart() {
    let voltageThreshold = 4.2;
    let temperatureThreshold = 100;

// 从 HTML 中获取输入并更新阈值
    // Function to update thresholds from external call
    window.updateChartThresholds = function(newVoltage, newTemp) {
        voltageThreshold = newVoltage;
        temperatureThreshold = newTemp;
    ...
    let isFault = dataCount >= 5; // 5次正常数据后转为故障
    ...
    if (dataCount < 30) {
        ...
        // 正常数据或故障数据
        temperature1.push(isFault ? generateRandomValue(20, 40) : generateRandomValue(20, 40));
        temperature2.push(isFault ? generateRandomValue(20, 40) : generateRandomValue(20, 40));
    ...
    // Refresh thresholds when returning to the page
    // 页面加载时获取当前阈值
    fetch('/threshold')
        .then(res => res.json())
        .then(data => {
            console.log('初始阈值:', data); // 调试输出
            if (window.updateChartThresholds) {
                window.updateChartThresholds(data.voltage, data.temperature);
            }
        });
}
